...
//   GET  /live:  Returns auctions that are currently live.
...
std::string verify_jwt_token(const std::string &token)
{
    try
    {
        ...
        // Use the builder pattern for verification.
        jwt::verify()
...
http::response<http::string_body> handle_create_listing(http::request<http::string_body> const &req)
{
    ...
    try
    {
        ...
        W.exec("INSERT INTO Auctions (seller_id, item_name, description, image_url, base_price, start_delay, live_duration) VALUES ($1, $2, $3, $4, $5, $6, $7)",
...
http::response<http::string_body> handle_make_live(http::request<http::string_body> const &req)
{
    ...
    try
    {
        ...
        W.exec("UPDATE Auctions SET start_delay = 0 WHERE auction_id = $1", pqxx::params(auction_id));
        W.commit();
        json res_json;
...
http::response<http::string_body> handle_my_listings(http::request<http::string_body> const &req)
{
    ...
    try
    {
        ...
        if (user_res.empty())
            return make_response(req, 404, "User not found");
        int seller_id = user_res[0]["user_id"].as<int>();
        auto result = W.exec("SELECT auction_id, item_name, description, image_url, base_price, posted_time, start_delay, live_duration FROM Auctions WHERE seller_id = $1", pqxx::params(seller_id));
...
http::response<http::string_body> handle_live(http::request<http::string_body> const &req)
{
    try
    {
        ...
        if (!C.is_open())
            return make_response(req, 500, "Database connection failed");
        pqxx::work W(C);
        auto result = W.exec("SELECT auction_id, seller_id, item_name, description, image_url, base_price, posted_time, start_delay, live_duration FROM Auctions WHERE (posted_time + (start_delay || ' minutes')::interval) <= CURRENT_TIMESTAMP AND CURRENT_TIMESTAMP < (posted_time + (start_delay || ' minutes')::interval + (live_duration || ' minutes')::interval)");
        json listings = json::array();
        for (auto row : result)
        {
            json auction;
...
http::response<http::string_body> handle_auction_result(http::request<http::string_body> const &req)
{
    try
    {
        ...
        if (result.empty())
        {
            res_json["message"] = "No bids placed";
        }
        else
        {
            res_json["bid_amount"] = result[0]["bid_amount"].as<double>();
